...
def ensure_goma_installed():
    ...
    if not os.path.exists(os.path.join(goma_dir, "compiler_proxy")):
        os.chdir(goma_dir)
        os.system(
            "curl https://clients5.google.com/cxx-compiler-service/download/goma_ctl.py "
            "-o goma_ctl.py"
        )
...
def common_gn_args():
    ...
    if is_goma_running():
        ...
        print(
            COLOR_FORMAT_YELLOW_STRING.format(
...
def monitor_compile_progress(child_process):
    ...
    for stdout_line in iter(child_process.stdout.readline, ""):
        ...
        if parsed:
            progress_ten_thousandths = int(
                float(parsed.group(1)) / float(parsed.group(2)) * 10000
            )
            ...
            percent = float(progress_ten_thousandths) / 100.0
            now_ms = int(time.time() * 1000)
            percent_per_second = percent / ((now_ms - start_ms) / 1000)
            if percent_per_second == 0:
                percent_per_second = 0.0001
            estimated_remaining_seconds = int((100.0 - percent) / percent_per_second)
...
def get_last_revision_number_for_cl(cl):
    url = (
        "https://chromium-review.googlesource.com/changes/chromium%2Fsrc~"
        + str(cl)
        + "/detail"
    )
...
def get_issue_number():
    ...
    if matches:
        ...
        print(
            "This branch isn't associated with any CL. "
...
def find_all_non_third_party_targets(options):
    ...
    os.chdir(get_chromium_src_dir())
    all_targets = (
        subprocess.check_output(shlex.split("gn ls  " + get_out_dir()))
        .decode("utf-8")
        .split("\n")
    )
    # Remove the leading "//"
    ...
    all_build_targets = [
        ...
        if (t.count("/") == 0 and not t.startswith("third_party"))
    ]
...
def find_all_test_targets(options):
    ...
    # Remove the leading "//"
    all_test_targets = sorted(
        [t[2:] for t in all_targets if (t.endswith("tests") or t.endswith("unittest"))]
    )
...
def ensure_depot_tools(options):
    ...
    if not os.path.exists(DEPOT_TOOLS):
        print(
            "I wasn't able to fetch the depot tools. Aborting. "
...
def get_repos_with_local_changes():
    ...
    for repo in repos:
        # TODO: multithread
        child = subprocess.Popen(["git", "diff", "--exit-code", "--quiet"], cwd=repo)
